...
def main():
    blue_blobs = [BlueBlob(WIDTH,HEIGHT) for i in range(STARTING_BLUE_BLOBS)] # List comprehension for creating list of blue blobs
    blue_blobs = dict(enumerate((blue_blobs))) # Change into a dictionary with numbers being ID's starting at 0
    red_blobs = [BlueBlob(WIDTH,HEIGHT,STEP) for i in range(STARTING_RED_BLOBS)] # List comprehension for create list of red blobs
